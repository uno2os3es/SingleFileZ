!(function () {
  'use strict';
  const e = new Map(),
    t = 'x-single-file-request-id',
    r = 8388608;
  async function s(e, t, s) {
    for (let a = 0; a * r <= s.array.length; a++) {
      const n = {
      ...
      ((n.truncated = s.array.length > r),
        n.truncated
          ? ((n.finished = (a + 1) * r > s.array.length),
            (n.array = s.array.slice(a * r, (a + 1) * r)))
          : (n.array = s.array),
        await browser.tabs.sendMessage(e, n));
    }
    return {};
  }
  function a(r, s = {}, n) {
    return new Promise((i, o) => {
      ...
      if (
        ...
        (d.onerror = (e) => o(new Error(e.detail))),
        (d.onreadystatechange = () => {
          d.readyState == XMLHttpRequest.DONE &&
            (d.status || d.response.byteLength
              ? (401 != d.status && 403 != d.status && 404 != d.status) || n
                ? i({
                    arrayBuffer: d.response,
                    array: Array.from(new Uint8Array(d.response)),
                    headers: {
                      'content-type': d.getResponseHeader('Content-Type'),
                    },
                    status: d.status,
                  })
                : a(r, s, !0).then(i).catch(o)
              : o(new Error('Empty response')));
        }),
        d.open('GET', r, !0),
        s.headers)
      )
        for (const e of Object.entries(s.headers))
          d.setRequestHeader(e[0], e[1]);
      if (n) {
        const r = String(Math.random()).substring(2);
        ((f = r), (u = s.referrer), e.set(f, u), d.setRequestHeader(t, r));
      }
      var f, u;
      d.send();
    });
      ...
      return new Promise((r) => {
        (async function (e, t) {
          if ('singlefile.fetch' == e.method)
            try {
              const r = await a(e.url, {
                referrer: e.referrer,
                headers: e.headers,
              });
              return s(t.tab.id, e.requestId, r);
            } catch (r) {
              return s(t.tab.id, e.requestId, { error: r.message, arrray: [] });
            }
          else if ('singlefile.fetchFrame' == e.method)
            return browser.tabs.sendMessage(t.tab.id, e);
        })(e, t)
          .then(r)
          .catch((e) => r({ error: e && e.toString() }));
      });
  }),
    browser.runtime.onMessage.addListener((e, t) => {
      if (
