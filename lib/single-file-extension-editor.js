  ...
  overflow: auto;
    ...
    async function init({ content, password }, { filename, reset } = {}) {
      ...
      if (detectSavedPage(contentDocument)) {
        ...
        const iconElement =
          origContentDocument.querySelector('link[rel*=icon]');
        if (iconElement) {
    ...
    async function initConstants() {
      [NOTES_WEB_STYLESHEET, MASK_WEB_STYLESHEET, HIGHLIGHTS_WEB_STYLESHEET] =
        await Promise.all([
          minifyText(
            await (await fetch('../pages/editor-note-web.css')).text()
          ),
          minifyText(
            await (await fetch('../pages/editor-mask-web.css')).text()
          ),
          minifyText(
            await (await fetch('../pages/editor-frame-web.css')).text()
          ),
        ]);
    }
    ...
    function addNote({ color }) {
      ...
      let positionX =
        NOTE_INITIAL_WIDTH +
        NOTE_INITIAL_POSITION_X -
        1 -
        boundingRectDocument.x;
      let positionY =
        NOTE_INITIAL_HEIGHT +
    ...
    function anchorNote(event, noteElement, deltaX, deltaY) {
      ...
      while (currentElement.parentElement && !positionedElement) {
        if (
          !FORBIDDEN_TAG_NAMES.includes(currentElement.tagName.toLowerCase())
        ) {
    ...
    function highlightSelection() {
      ...
      for (
        let indexRange = 0;
        indexRange < selection.rangeCount;
        indexRange++
      ) {
            ...
            while (treeWalker.nextNode()) {
              ...
              if (treeWalker.currentNode == range.startContainer) {
                if (
                  ...
                  range.startContainer.TEXT_NODE
                ) {
              ...
              if (treeWalker.currentNode == range.endContainer) {
                if (
                  range.endContainer.nodeType == range.endContainer.TEXT_NODE
                ) {
    ...
    function formatPage(applySystemTheme) {
      ...
      document.querySelectorAll(NOTE_TAGNAME).forEach((containerElement) => {
        ...
        const className =
          'singlefile-note-id-' + containerElement.dataset.noteId;
