...
async function createMenus(tab) {
  ...
  if (BROWSER_MENUS_API_SUPPORTED && options) {
    ...
    if (Object.keys(profiles).length > 1) {
      ...
      const currentProfileChecked =
        !rule || rule.profile == config.CURRENT_PROFILE_NAME;
      menus.create({
        id: currentProfileId,
        type: 'radio',
        contexts: defaultContexts,
      ...
      const associatedDefaultProfileChecked =
        Boolean(rule) && rule.profile == config.DEFAULT_PROFILE_NAME;
      menus.create({
  ...
  if (pendingRefresh) {
    ...
    (await browser.tabs.query({})).forEach(
      async (tab) => await refreshTab(tab)
    );
  }
}
