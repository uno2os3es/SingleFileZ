...
const highlightYellowButton = document.querySelector(
...
const toggleHighlightsButton = document.querySelector(
  '.toggle-highlights-button'
);
const removeHighlightButton = document.querySelector(
...
toggleHighlightsButton.title = browser.i18n.getMessage(
...
addYellowNoteButton.onmouseup = () =>
  editorElement.contentWindow.postMessage(
    JSON.stringify({ method: 'addNote', color: 'note-yellow' }),
    '*'
  );
addPinkNoteButton.onmouseup = () =>
  editorElement.contentWindow.postMessage(
    JSON.stringify({ method: 'addNote', color: 'note-pink' }),
    '*'
  );
addBlueNoteButton.onmouseup = () =>
  editorElement.contentWindow.postMessage(
    JSON.stringify({ method: 'addNote', color: 'note-blue' }),
    '*'
  );
addGreenNoteButton.onmouseup = () =>
  editorElement.contentWindow.postMessage(
    JSON.stringify({ method: 'addNote', color: 'note-green' }),
    '*'
  );
...
toggleNotesButton.onmouseup = () => {
  if (
    toggleNotesButton.getAttribute('src') ==
    '/src/ui/resources/button_note_visible.png'
  ) {
    toggleNotesButton.src = '/src/ui/resources/button_note_hidden.png';
    editorElement.contentWindow.postMessage(
      JSON.stringify({ method: 'hideNotes' }),
      '*'
    );
  } else {
    toggleNotesButton.src = '/src/ui/resources/button_note_visible.png';
    editorElement.contentWindow.postMessage(
      JSON.stringify({ method: 'displayNotes' }),
      '*'
    );
  }
};
toggleHighlightsButton.onmouseup = () => {
  if (
    toggleHighlightsButton.getAttribute('src') ==
    '/src/ui/resources/button_highlighter_visible.png'
  ) {
    toggleHighlightsButton.src =
      '/src/ui/resources/button_highlighter_hidden.png';
    editorElement.contentWindow.postMessage(
      JSON.stringify({ method: 'hideHighlights' }),
      '*'
    );
  } else {
    displayHighlights();
...
removeHighlightButton.onmouseup = () => {
  ...
  if (removeHighlightButton.classList.contains('remove-highlight-disabled')) {
    ...
    displayHighlights();
    editorElement.contentWindow.postMessage(
      JSON.stringify({ method: 'enableRemoveHighlights' }),
      '*'
    );
    editorElement.contentWindow.postMessage(
...
if (typeof print == 'function') {
  printPageButton.onmouseup = () => {
    editorElement.contentWindow.postMessage(
      JSON.stringify({ method: 'printPage' }),
      '*'
    );
  };
} else {
...
addEventListener('message', (event) => {
  ...
  if (message.method == 'setContent') {
    ...
    getContentPageData(tabData.content, message.content, {
  ...
  if (message.method == 'onInit') {
    ...
    document.title = '[SingleFileZ] ' + message.title;
    if (message.filename) {
  ...
  if (message.method == 'displayInfobar') {
    ...
    infobarElement.shadowRoot.querySelector('style').textContent +=
...
browser.runtime.onMessage.addListener((message) => {
  ...
  if (message.method == 'editor.setTabData') {
    ...
    if (!message.truncated || message.finished) {
      ...
      editorElement.contentWindow.focus();
      setInterval(
        () => browser.runtime.sendMessage({ method: 'editor.ping' }),
        15000
      );
    }
    return Promise.resolve({});
...
async function downloadContent(message) {
  ...
  if (result.done) {
    ...
    return browser.runtime
      .sendMessage({ method: 'downloads.end', taskId: result.value.taskId })
      .then(() => ({}));
  } else {
    return Promise.resolve({});
  }
...
async function getContentPageData(zipContent, page, options) {
  ...
  const zipReader = new zip.ZipReader(
    new zip.Uint8ArrayReader(new Uint8Array(zipContent))
  );
...
function getPageData(resources) {
  ...
  for (const resource of resources) {
    ...
    const filename = resource.filename.substring(
      resourcePageData.prefixPath.length
    );
...
function getPageDataResource(resource, prefixPath = '', pageData) {
  ...
  if (filename.startsWith('frames/')) {
    ...
    if (!pageData.resources.frames[framesIndex]) {
      pageData.resources.frames[framesIndex] = Object.assign(
        JSON.parse(JSON.stringify(EMPTY_PAGE_DATA)),
        {
          name: framePath,
    ...
    return getPageDataResource(
      resource,
      prefixPath + framePath,
      pageData.resources.frames[framesIndex]
    );
  } else {
    return pageData;
  }
