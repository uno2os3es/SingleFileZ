...
const SELECTED_CONTENT_ATTRIBUTE_NAME =
...
const SINGLE_FILE_UI_ELEMENT_CLASS =
  singlefile.helper.SINGLE_FILE_UI_ELEMENT_CLASS;
const SELECT_PX_THRESHOLD = 8;
const LOG_PANEL_DEFERRED_IMAGES_MESSAGE = browser.i18n.getMessage(
  'logPanelDeferredImages'
);
const LOG_PANEL_FRAME_CONTENTS_MESSAGE = browser.i18n.getMessage(
  'logPanelFrameContents'
);
const LOG_PANEL_EMBEDDED_IMAGE_MESSAGE = browser.i18n.getMessage(
...
function onLoadingDeferResources(options) {
  updateLog(
    ...
    LOG_PANEL_DEFERRED_IMAGES_MESSAGE,
    '…',
    options
  );
}
...
function onInsertingEmbeddedImage(options) {
  updateLog(
    ...
    LOG_PANEL_EMBEDDED_IMAGE_MESSAGE,
    '…',
    options
  );
}
...
function onStartStage(step, options) {
  updateLog(
    'step-' + step,
    `${LOG_PANEL_STEP_MESSAGE} ${step + 1} / 3`,
    '…',
    options
  );
}
  ...
  for (let indexRange = 0; indexRange < selection.rangeCount; indexRange++) {
    ...
    if (range && range.commonAncestorContainer) {
      ...
      while (!finished) {
        if (
          ...
          if (
            ...
            selectionFound = true;
            if (
              treeWalker.currentNode.tagName == 'A' &&
              treeWalker.currentNode.href
            ) {
      ...
      if (
        selectionFound &&
        treeWalker.currentNode == range.endContainer &&
        treeWalker.currentNode.querySelectorAll
      ) {
        treeWalker.currentNode
          .querySelectorAll('*')
          .forEach((descendantElement) => {
  ...
  for (let indexRange = 0; indexRange < selection.rangeCount; indexRange++) {
    ...
    if (range && range.commonAncestorContainer) {
      ...
      if (
        ...
        treeWalker.currentNode
          .querySelectorAll('*')
          .forEach((descendantElement) => markSelectedNode(descendantElement));
      }
    }
  }
  return selectionFound;
...
function unmarkSelection() {
  document
    .querySelectorAll('[' + SELECTED_CONTENT_ATTRIBUTE_NAME + ']')
    .forEach((selectedContent) =>
      selectedContent.removeAttribute(SELECTED_CONTENT_ATTRIBUTE_NAME)
    );
}
...
function selectArea() {
  return new Promise((resolve) => {
    ...
    function cleanupSelectionRanges() {
      const selection = getSelection();
      for (
        let indexRange = selection.rangeCount - 1;
        indexRange >= 0;
        indexRange--
      ) {
    ...
    function saveSelectedRanges() {
      ...
      for (
        let indexRange = 0;
        indexRange < getSelection().rangeCount;
        indexRange++
      ) {
...
function getTarget(event) {
  ...
  if (newTarget == target) {
    newTarget = determineTargetElement(
      'ceil',
      target,
      boundingRect.left + boundingRect.width - event.clientX,
  ...
  if (newTarget == target) {
    newTarget = determineTargetElement(
      'ceil',
      target,
      boundingRect.top + boundingRect.height - event.clientY,
  ...
  while (
    target &&
    target.clientWidth <= SELECT_PX_THRESHOLD &&
    target.clientHeight <= SELECT_PX_THRESHOLD
  ) {
...
function moveAreaSelector(target) {
  requestAnimationFrame(() => {
    ...
    selectorElement.style.setProperty(
      'top',
      scrollingElement.scrollTop + boundingRect.top - 10 + 'px'
    );
    selectorElement.style.setProperty(
      'left',
      scrollingElement.scrollLeft + boundingRect.left - 10 + 'px'
    );
    selectorElement.style.setProperty('width', boundingRect.width + 20 + 'px');
    selectorElement.style.setProperty(
      'height',
      boundingRect.height + 20 + 'px'
    );
  });
}
...
function createMaskElement() {
  try {
    ...
    if (!maskElement) {
      ...
      maskElementContent.style.setProperty('opacity', 0.3);
      maskElement.offsetWidth;
    }
    return maskElement;
  } catch (error) {
...
function createProgressBarElement(maskElement) {
  try {
    let progressBarElement = maskElement.shadowRoot.querySelector(
      '.' + PROGRESSBAR_CLASSNAME
    );
...
function updateLog(id, textContent, textStatus, options) {
  try {
    if (options.logsEnabled) {
      const logsContentElement = logsWindowElement.shadowRoot.querySelector(
        '.' + LOGS_CLASSNAME
      );
      let lineElement = logsContentElement.querySelector(
        "[data-id='" + id + "']"
      );
      if (!lineElement) {
...
function updateProgressBar(index, maxIndex) {
  try {
    const maskElement = document.querySelector(MASK_TAGNAME);
    if (maskElement) {
      const progressBarElement = maskElement.shadowRoot.querySelector(
        '.' + PROGRESSBAR_CLASSNAME
      );
...
function determineTargetElement(
  ...
  if (
    Math[roundingMethod](widthDistance / SELECT_PX_THRESHOLD) <= parents.length
  ) {
    target =
      parents[
        parents.length -
