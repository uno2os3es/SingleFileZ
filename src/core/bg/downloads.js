...
async function downloadTabPage(message, tab) {
  ...
  if (message.blobURL) {
    try {
      message.pageData = await yabson.parse(
        new Uint8Array(await (await fetch(message.blobURL)).arrayBuffer())
      );
    ...
    if (message.data) {
      ...
      const message = await yabson.parse(
  ...
  async function download(message) {
    try {
      let skipped;
      if (
        message.backgroundSave &&
        !message.saveToGDrive &&
        !message.saveToDropbox &&
        !message.saveWithWebDAV &&
        !message.saveToGitHub
      ) {
      ...
      if (skipped) {
        ...
        if (message.openEditor) {
          ...
          await downloadPageForeground(
            ...
            blob,
            tabId,
            message.foregroundSave
          );
        } else if (message.saveWithWebDAV) {
          response = await saveWithWebDAV(
            message.taskId,
            ...
            { filenameConflictAction: message.filenameConflictAction, prompt }
          );
          ...
          await saveToGDrive(
            ...
            },
            {
              onProgress: (offset, size) =>
                ui.onUploadProgress(tabId, offset, size),
          ...
          if (message.backgroundSave) {
            ...
            response = await downloadPage(message, {
              ...
              filenameReplacementCharacter:
                message.filenameReplacementCharacter,
...
async function saveToGitHub(
  ...
  try {
    ...
    if (!taskInfo || !taskInfo.cancelled) {
      ...
      return await client.upload(filename, blob, {
...
async function saveToGDrive(
  ...
  try {
    ...
    const taskInfo = business.getTaskInfo(taskId);
    if (!taskInfo || !taskInfo.cancelled) {
      return await gDrive.upload(filename, blob, uploadOptions, (callback) =>
...
async function saveToDropbox(taskId, filename, blob, uploadOptions) {
  try {
    ...
    const taskInfo = business.getTaskInfo(taskId);
    if (!taskInfo || !taskInfo.cancelled) {
      return await dropbox.upload(filename, blob, uploadOptions, (callback) =>
        business.setCancelCallback(taskId, callback)
      );
    }
  } catch (error) {
...
async function downloadPageForeground(
  taskId,
  filename,
  content,
  tabId,
