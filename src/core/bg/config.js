...
const BACKGROUND_SAVE_SUPPORTED = !(
...
async function upgrade() {
  ...
  for (const profileName of profileNames) {
    ...
    if (!profile._migratedTemplateFormat) {
      profile.filenameTemplate = updateFilenameTemplate(
...
async function getConfig() {
  ...
  const { maxParallelWorkers, processInForeground } = await configStorage.get([
    'maxParallelWorkers',
    'processInForeground',
  ]);
...
async function getProfiles() {
  ...
  Object.keys(profiles).forEach(
    (profileName) =>
      (result[profileName.substring(PROFILE_NAME_PREFIX.length)] =
...
async function updateProfile(profileName, profile) {
  ...
  Object.keys(previousProfile).forEach((key) => {
    profile[key] =
      profile[key] === undefined ? previousProfile[key] : profile[key];
  });
  await setProfile(profileName, profile);
}
...
async function renameProfile(oldProfileName, profileName) {
  ...
  await configStorage.set({
    [PROFILE_NAME_PREFIX + profileName]: profile,
    rules,
  });
}
...
async function getProfileNames() {
  return Object.keys(await configStorage.get())
    .filter((key) => key.startsWith(PROFILE_NAME_PREFIX))
    .map((key) => key.substring(PROFILE_NAME_PREFIX.length));
}

async function getProfileKeyNames() {
  return Object.keys(await configStorage.get()).filter((key) =>
...
async function importConfig(config) {
  ...
  Object.keys(config.profiles).forEach(
    (profileName) =>
      (newConfig[PROFILE_NAME_PREFIX + profileName] =
