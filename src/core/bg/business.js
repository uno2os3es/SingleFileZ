...
const ERROR_CHANNEL_CLOSED_CHROMIUM =
...
const extensionScriptFiles = ['lib/single-file-extension.js'];
...
async function initMaxParallelWorkers() {
  if (!maxParallelWorkers) {
    ...
    maxParallelWorkers = processInForeground
      ? 1
      : configData.maxParallelWorkers;
  }
}
...
function isIgnoredError(error) {
  return (
    ...
    error.message.startsWith(
      ERROR_EDITOR_PAGE_CHROMIUM + JSON.stringify(editor.EDITOR_URL)
    )
  );
}
