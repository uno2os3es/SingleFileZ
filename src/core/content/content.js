...
import {
  fetch,
  frameFetch,
} from './../../lib/single-file/fetch/content/content-fetch.js';
import * as ui from './../../ui/content/content-ui.js';
import {
  onError,
  getOpenFileBar,
  openFile,
} from './../../ui/common/common-content-ui.js';
...
async function savePage(message) {
  ...
  if (!processing && (!bootstrap || !bootstrap.pageInfo.processing)) {
    options.updatedResources = bootstrap
      ? bootstrap.pageInfo.updatedResources
      : {};
        ...
        if (pageData) {
          if (
            (!options.backgroundSave ||
              ...
              options.saveToDropbox) &&
            options.confirmFilename
          ) {
            pageData.filename =
              ui.prompt('Save as', pageData.filename) || pageData.filename;
          }
...
async function processPage(options) {
  ...
  options.onprogress = (event) => {
    if (!processor.cancelled) {
      ...
      if (
        event.type == event.RESOURCES_INITIALIZED ||
        event.type == event.RESOURCE_LOADED
      ) {
      ...
      if (options.loadDeferredImages) {
        frameTreePromise = new Promise((resolve) =>
          globalThis.setTimeout(
            ...
            options.loadDeferredImagesBeforeFrames ||
              !options.loadDeferredImages
              ? 0
              : options.loadDeferredImagesMaxIdleTime
          )
        );
      } else {
  ...
  if (!options.loadDeferredImagesBeforeFrames && !processor.cancelled) {
    [options.frames] = await new Promise((resolve) => {
      const preInitializationAllPromises = Promise.all(
        preInitializationPromises
      );
      processor.cancel = function () {
        cancelProcessor();
        resolve([[]]);
      };
      preInitializationAllPromises.then(() =>
  ...
  if (options.delayBeforeProcessing) {
    await new Promise((resolve) =>
      setTimeout(resolve, options.delayBeforeProcessing * 1000)
    );
  }
  framesSessionId = options.frames && options.frames.sessionId;
  const selectedFrame =
    options.frames &&
    options.frames.find((frameData) => frameData.requestedFrame);
  ...
  if (!processor.cancelled) {
    if (options.confirmInfobarContent) {
      options.infobarContent =
        ui.prompt('Infobar content', options.infobarContent) || '';
    }
