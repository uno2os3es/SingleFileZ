...
browser.runtime.onMessage.addListener((message) => {
  if (
    (autoSaveEnabled && message.method == 'content.autosave') ||
...
function executeBootstrap(data) {
  ...
  scriptElement.textContent =
    ...
    'function run(){this.bootstrap([' +
    new Uint8Array(data).toString() +
...
function init() {
  ...
  if (
    ...
    !singlefile.pageInfo.processing
  ) {
    pageAutoSaved = false;
    previousLocationHref = location.href;
    browser.runtime
      .sendMessage({
        method: 'tabs.init',
        savedPageDetected: detectSavedPage(document),
      })
      .catch(() => {});
...
async function autoSavePage() {
  ...
  if ((!autoSavingPage || autoSaveTimeout) && !pageAutoSaved) {
    ...
    if (optionsAutoSave.autoSaveDelay && !autoSaveTimeout) {
      await new Promise(
        (resolve) =>
          (autoSaveTimeout = setTimeout(
            resolve,
            optionsAutoSave.autoSaveDelay * 1000
          ))
      );
      await autoSavePage();
    } else {
      const waitForUserScript =
        globalThis[helper.WAIT_FOR_USERSCRIPT_PROPERTY_NAME];
      let frames = [];
      let framesSessionId;
      autoSaveTimeout = null;
      if (
        !optionsAutoSave.removeFrames &&
        globalThis.frames &&
        globalThis.frames.length
      ) {
        frames =
          await singlefile.processors.frameTree.getAsync(optionsAutoSave);
      }
      ...
      const docData = helper.preProcessDoc(
        document,
        globalThis,
        optionsAutoSave
      );
      ...
      helper.postProcessDoc(
        document,
        docData.markedElements,
        docData.invalidElements
      );
      if (optionsAutoSave.userScriptEnabled && waitForUserScript) {
...
function refresh() {
  if (
    autoSaveEnabled &&
    optionsAutoSave &&
    (optionsAutoSave.autoSaveUnload ||
      optionsAutoSave.autoSaveLoadOrUnload ||
      optionsAutoSave.autoSaveDiscard ||
      optionsAutoSave.autoSaveRemove)
  ) {
...
function onUnload() {
  if (
    ...
    autoSaveUnloadedPage({
      autoSaveUnload: optionsAutoSave.autoSaveUnload,
      autoSaveRemove: optionsAutoSave.autoSaveRemove,
    });
  }
}

function autoSaveUnloadedPage({
  autoSaveUnload,
  autoSaveDiscard,
  autoSaveRemove,
}) {
...
function savePage(
  docData,
  frames,
  { autoSaveUnload, autoSaveDiscard, autoSaveRemove } = {}
) {
  const helper = singlefile.helper;
  const updatedResources = singlefile.pageInfo.updatedResources;
  const visitDate = singlefile.pageInfo.visitDate.getTime();
  Object.keys(updatedResources).forEach(
    (url) => (updatedResources[url].retrieved = false)
  );
  browser.runtime.sendMessage({
...
async function openEditor() {
  ...
  for (
    ...
    if (message.truncated) {
      ...
      message.content =
        content instanceof Uint8Array ? Array.from(content) : content;
    }
...
function detectSavedPage(document) {
  if (savedPageDetected === undefined) {
    ...
    savedPageDetected =
      document.documentElement.dataset.sfz == '' ||
