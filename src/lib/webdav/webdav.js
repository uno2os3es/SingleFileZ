...
class WebDAV {
  constructor(url, username, password) {
    ...
    this.url = url;
    this.authorization =
      BASIC_PREFIX_AUTHORIZATION +
      btoa(username + AUTHORIZATION_SEPARATOR + password);
  }
...
async function upload(filename, content, options) {
    ...
    if (filenameConflictAction == CONFLICT_ACTION_OVERWRITE) {
      ...
      if (response.status == FOUND_STATUS) {
        if (
          ...
          (filenameConflictAction == CONFLICT_ACTION_PROMPT && !prompt)
        ) {
          const { filenameWithoutExtension, extension, indexFilename } =
            splitFilename(filename);
          options.indexFilename = indexFilename + 1;
          return await upload(
            getFilename(filenameWithoutExtension, extension),
            content,
            options
          );
  ...
  function sendRequest(path, method, body) {
    ...
    return fetch(url + path, {
      ...
      signal,
      body,
      credentials: CREDENTIALS_PARAMETER,
    });
  }
  ...
  function splitFilename(filename) {
    ...
    let indexFilename;
    ({ filenameWithoutExtension, indexFilename } = extractIndexFilename(
  ...
  function extractIndexFilename(filenameWithoutExtension) {
    const indexFilenameMatch = filenameWithoutExtension.match(
      INDEX_FILENAME_REGEXP
    );
    let indexFilename = 0;
    if (indexFilenameMatch && indexFilenameMatch.length > 1) {
      const parsedIndexFilename = Number(
        indexFilenameMatch[indexFilenameMatch.length - 1]
      );
