...
browser.runtime.onMessage.addListener((message) => {
  if (
    message.method == 'singlefile.fetchFrame' &&
    window.frameId &&
    ...
    if (message.error) {
      ...
      if (!message.truncated || message.finished) {
        pendingResponse.resolve({
          ...
          headers: {
            get: (headerName) => message.headers && message.headers[headerName],
          },
  ...
  const result = new Promise((resolve, reject) => {
    document.dispatchEvent(
      new CustomEvent(FETCH_REQUEST_EVENT, {
        detail: JSON.stringify({ url, options }),
      })
    );
...
export { fetchResource as fetch, frameFetch };

async function fetchResource(url, options = {}) {
  try {
    const fetchOptions = { cache: 'force-cache', headers: options.headers };
    return await (options.referrer && USE_HOST_FETCH
      ? hostFetch(url, fetchOptions)
