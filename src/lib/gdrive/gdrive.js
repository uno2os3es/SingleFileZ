  ...
  async auth(options = { interactive: true }) {
    if (nativeAuth(options)) {
      ...
      this.authURL =
        ...
        this.scopes.join(' ');
      return options.code
        ? authFromCode(this, options)
        : initAuth(this, options);
    }
  }
...
class MediaUploader {
  ...
  async upload(indexFilename = 1) {
    let method = 'POST';
    let fileId;
    const httpListResponse = getResponse(
      await fetch(
        GDRIVE_URL +
    ...
    if (response.files.length) {
      if (this.filenameConflictAction == CONFLICT_ACTION_OVERWRITE) {
        ...
        const name =
          nameWithoutExtension + ' (' + indexFilename + ').' + extension;
        const httpResponse = getResponse(
          await fetch(
            GDRIVE_URL +
              `?q=name = '${name}' and trashed != true and '${this.metadata.parents[0]}' in parents`,
            {
              headers: {
                Authorization: 'Bearer ' + this.token,
                'Content-Type': 'application/json',
...
async function authFromCode(gdrive, options) {
  ...
  return {
    ...
    expirationDate: gdrive.expirationDate,
  };
}
...
async function initAuth(gdrive, options) {
  ...
  try {
    ...
    if (
      error.message &&
      (error.message == 'code_required' || error.message.includes('access'))
    ) {
      if (code) {
        options.code = code;
        return await authFromCode(gdrive, options);
  ...
  if (fileParts.length) {
    ...
    for (const folderName of fileParts) {
      ...
      if (folderId) {
        ...
        try {
          parentFolderId = await getOrCreateFolder(
            gdrive,
            folderName,
            parentFolderId
          );
