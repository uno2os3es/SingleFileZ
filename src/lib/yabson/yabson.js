...
class SerializerData {
  ...
  addObject(value) {
    this.objects.push(
      testReferenceable(value) && !testCircularReference(value, this)
        ? value
        : undefined
    );
  }
...
function getSerializer(value, { chunkSize = DEFAULT_CHUNK_SIZE } = {}) {
  ...
  function initPreviousData() {
    previousResult = new Promise(
      (resolve) => (resolvePreviousResult = resolve)
    );
  }
...
async function serializeSymbols(data, value) {
  ...
  const symbols = ownPropertySymbols.map((propertySymbol) => [
    propertySymbol,
...
async function serializeArray(data, array) {
  ...
  const notEmptyIndexes = Object.keys(array)
    .filter((key) => testInteger(Number(key)))
    .map((key) => Number(key));
  let indexNotEmptyIndexes = 0,
...
class ParserData {
  ...
  executeSetters() {
    this.setters.forEach(({ functionArguments, setterFunction }) => {
      const resolvedArguments = functionArguments.map((argument) =>
        testReference(argument) ? argument.getObject() : argument
      );
...
async function parseSymbols(data, value) {
  ...
  data.setObject([symbols], (symbols) =>
    symbols.forEach(
      ([symbol, propertyValue]) => (value[symbol] = propertyValue)
    )
  );
}
