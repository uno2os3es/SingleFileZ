...
const DROPBOX_APPEND_URL =
  'https://content.dropboxapi.com/2/files/upload_session/append_v2';
const DROPBOX_FINISH_URL =
...
class MediaUploader {
  ...
  async upload() {
    const httpListResponse = getResponse(
      await fetch(DROPBOX_SEARCH_URL, {
        method: 'POST',
        headers: {
...
async function authFromCode(dropbox, options) {
  ...
  return {
    ...
    expirationDate: dropbox.expirationDate,
  };
}
...
async function sendFile(mediaUploader) {
  ...
  const httpAppendResponse = getResponse(
    await fetch(DROPBOX_APPEND_URL, {
      method: 'POST',
      headers: {
    ...
    headers: {
      ...
      'Dropbox-API-Arg': stringify({
        ...
        commit: {
          path,
          mode:
            mediaUploader.filenameConflictAction == CONFLICT_ACTION_OVERWRITE
              ? 'overwrite'
              : 'add',
          autorename:
            mediaUploader.filenameConflictAction == CONFLICT_ACTION_UNIQUIFY,
        },
      }),
    },
  });
  if (httpFinishResponse.status == 200) {
    ...
    httpFinishResponse.status == 409 &&
    mediaUploader.filenameConflictAction == CONFLICT_ACTION_PROMPT
  ) {
...
function stringify(value) {
  return JSON.stringify(value).replace(ENCODED_CHARS, function (c) {
